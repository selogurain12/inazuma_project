...
import { Episode, EpisodeForm } from "./episodedata";
...
export function EpisodeEdit() {
    ...
    const { register, handleSubmit, control, reset, setValue, formState: { errors } } = useForm<EpisodeForm>();
    ...
    useEffect(() => {
        const fetchOptions = async () => {
            try {
                ...
                const episodeResponse = await axios.get(`${apiUrl}/episodes/${id}`);
                setEpisode(episodeResponse.data);
                if (episodeResponse.data) {
                    reset({
                        titre: episodeResponse.data.titre || "",
                        titre_original: episodeResponse.data.titre_original || "",
                        note: episodeResponse.data.note || "",
                        numepisode: episodeResponse.data.numepisode || "",
                        saison: episodeResponse.data.saison || "",
                        scenario: episodeResponse.data.scenario || "",
                        supertechniques: episodeResponse.data.supertechniques || [],
                        supertactiques: episodeResponse.data.supertactiques || [],
                        matchs: episodeResponse.data.matchs || [],
                        diffusion_originale: episodeResponse.data.episodes || "",
                        serie: episodeResponse.data.serie || null,
                        images: episodeResponse.data.images || [],
                        resume: episodeResponse.data.entraineur || "",
                        video: episodeResponse.data.video || "",
                        equipes: episodeResponse.data.equipes || []
                    });
                }
            } catch (error) {
    ...
    const onSubmit = async (data: EpisodeForm) => {            
        ...
        const apiUrl = import.meta.env.VITE_PUBLIC_API_URL;
        try {
            await axios.patch(`${apiUrl}/episodes/${id}`, {
                titre: data.titre,
