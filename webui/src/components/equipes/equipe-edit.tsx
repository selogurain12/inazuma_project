...
import { Equipe, EquipeForm } from "./equipedata";
import { MenuEquipe } from "./menu-equipes";
import { validationRules } from "./equiperules";

export function EquipeEdit() {
    const { id } = useParams<{ id: string }>();
    const { register, handleSubmit, control, reset, setValue, formState: { errors } } = useForm<EquipeForm>();
    ...
    return (
        ...
        <Box sx={{ flexGrow: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
            <Typography variant="h3" gutterBottom>
                Modifier {equipe?.nom_francais_equipe}
            </Typography>
        </Box>
